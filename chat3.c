/*
  ...
  TCP chat program ver.2(non-blocking version with multiple connections)
*/
#include <sys/types.h>
...
bool parseCommand(char *, struct Client *, struct Client *);
        ...
        for (client = clientRoot; client != NULL; client = client->next)
        {
            ...
            if (FD_ISSET(client->fd, &fdSet))
            {
                ...
                // The normal case when the incoming message is not something special
                {
                    ...
                    parseCommand(buf, client, clientRoot);
...
bool parseCommand(char *cmd, struct Client *client, struct Client *clientRoot)
{
    bool captured = false;
    ...
    while (cp != NULL)
    {
        ...
        if (strcasecmp(cmdName, NAME) == 0 && cp != NULL)
        {
            bool duplicated = false;
            struct Client *cln;
            for (cln = clientRoot; cln != NULL; cln = cln->next)
            {
                if (strcmp(cp, cln->name) == 0 && client->fd != cln->fd)
                {
                    char *dynBuf = (char *)calloc(BUF_SIZE, sizeof(char));
                    sprintf(dynBuf, "Name already exists!\n");
                    write(client->fd, dynBuf, strlen(dynBuf));
                    free(dynBuf);
                    duplicated = true;
                    break;
                }
